import type { ResourceArticle } from "./types";

export const lgbtqChurchResource: ResourceArticle = {
  slug: "lgbtq-people-and-harm-in-churches",
  title: "When church becomes a source of harm for LGBTQ+ people",
  deck:
    "An evidence-based review of how rejection, identity conflict, coercive teachings and exclusion in religious settings can affect LGBTQ+ mental health … and what safer, affirming faith communities can do differently.",
  category: "LGBTQ+ people and religious harm",
  readingTime: 16,
  intensity: "moderate",
  warnings: [
    "Spiritual abuse",
    "Coercive control",
    "Threats of punishment or damnation",
    "Conversion practices",
    "Family rejection",
    "Shunning",
    "Suicide or self-harm"
  ],
  image: "/images/lgbtq-church-refuge.svg",
  imageAlt:
    "Rainbow-coloured fabric with the words: The church should be a refuge, not a source of trauma. AfterTheChurch.",
  overview:
    "For many LGBTQ+ people, religion is not simply a source of harm or a source of comfort. It can be both. The strongest research does not support the idea that faith itself is inherently damaging. Instead, risk tends to rise when religious participation is tied to rejection, shame, concealment, identity conflict, threats of exclusion, pressure to change sexual orientation or gender identity, or the belief that a core part of oneself is morally defective. A 2026 systematic review synthesising 55 quantitative studies and more than 512,000 LGBTQIA+ participants described religion and spirituality as an ambivalent force: distress was more likely when faith was linked to internalised stigma, identity conflict and exclusion, while affirming communities, meaning-making and supportive spiritual coping could be protective. This article therefore focuses on the conditions under which churches can become harmful, while recognising that affirming faith communities can also be places of belonging and recovery.",
  keyPoints: [
    "The evidence points to minority stress and religious identity conflict … not LGBTQ+ identity itself … as important pathways to poorer mental health.",
    "In one national study of LGBT young adults, 43% reported conflict between their religious beliefs and sexuality; among those who experienced conflict, 42% said they left their religious affiliation because of it.",
    "A large study of college students found sexual-minority young adults had 4.52 times the adjusted odds of past-year suicidal ideation compared with heterosexual peers; among Catholics, the adjusted odds ratio was 5.25.",
    "A 2025 mixed-methods study of 429 LGBTQ+ participants found religion played a role in suppressing sexual orientation for 59.2% of participants.",
    "A 4-year longitudinal study of LGBTQ+ Latter-day Saints found about two thirds of participants religiously deidentified to some degree, showing how sustained identity conflict can reshape religious belonging over time.",
    "The relationship is not one-directional: affirming religious affiliation has been associated with lower depressive impact from discrimination, and supportive religious climates have been linked with healthier outcomes for sexual-minority youth.",
    "Churches reduce harm when they remove coercion, protect confidentiality, reject humiliation and forced disclosure, make support available without requiring identity suppression, and create genuine routes for complaint and accountability.",
    "This article synthesises peer-reviewed evidence; it has not itself undergone independent academic peer review."
  ],
  fullSections: [
    {
      heading: "The central issue is not religion alone … it is the social environment created around LGBTQ+ identity",
      paragraphs: [
        "Religion is often associated with community, meaning, ritual and social support. Those benefits can be psychologically valuable. But the same institution can become a source of chronic stress when an LGBTQ+ person learns that acceptance is conditional on concealment, celibacy, silence, gender conformity or attempts to change. Minority stress theory helps explain why: repeated exposure to stigma can produce expectations of rejection, internalised stigma, concealment and heightened vigilance. Religious settings can intensify these mechanisms when moral authority and belonging are attached to them.",
        "A recent systematic review of 55 quantitative studies, involving 512,551 LGBTQIA+ participants, found that religion and spirituality were associated with distress when they were linked to internalised homo-, bi- or transphobia, identity conflict and exclusion. The same review also found evidence of resilience when spirituality provided meaning, coping and affirming community. This distinction matters. It means the question is not simply whether someone is religious, but what kind of religious environment they are living in and what that environment requires of them."
      ]
    },
    {
      heading: "Religious and LGBTQ+ identity conflict can become a mental-health stressor",
      paragraphs: [
        "One of the clearest patterns in the literature is conflict between a person's religious identity and their sexual or gender identity. In a national study of LGBT young adults, 43% reported growing up in a religious community and experiencing conflict between religious beliefs and sexuality. Of the full sample, 12% reported that the conflict remained unresolved. Among participants who experienced conflict, 42% had left their religious affiliation because of it. These figures should not be treated as universal prevalence estimates, but they show that religious conflict is common enough to be a major clinical and pastoral concern.",
        "Qualitative research among gay Catholics has described the same tension in personal terms: participants reported trying to reject same-sex attraction, reducing religious involvement, questioning doctrine, challenging clerical authority, seeking professional help or trying to find accepting clergy. The psychological burden is not simply disagreement with a doctrine. It can involve the feeling that two deeply important parts of the self have been made mutually incompatible."
      ]
    },
    {
      heading: "Shame, concealment and internalised stigma are important pathways of harm",
      paragraphs: [
        "When a church repeatedly describes same-sex relationships, gender diversity or LGBTQ+ identity as disordered, sinful, rebellious or evidence of spiritual failure, those messages can move from the social environment into a person's self-concept. Research has linked religious stress to more negative sexual-minority identity and greater psychological difficulty. Studies of religious sexual-minority adults likewise identify internalised homonegativity as an important mechanism connecting non-affirming environments with distress.",
        "A 2025 mixed-methods study of 429 LGBTQ+ people found that religion influenced suppression of sexual orientation for 254 participants … 59.2% of the sample. Participants described fear, shame, rejection and separation from religious communities. Concealment can sometimes be a rational short-term safety strategy, particularly where housing, family, immigration status, employment or education are tied to the church. The harm occurs when concealment becomes the price of belonging and the person has little realistic freedom to be known safely."
      ]
    },
    {
      heading: "Suicidality requires careful interpretation … but the disparities are serious",
      paragraphs: [
        "Suicide is multi-causal, and no responsible article should claim that a church teaching by itself causes a suicide attempt. The evidence does, however, show strong associations between sexual-minority status, stigma, identity conflict and suicidal thinking. In a study comparing 37,471 heterosexual and 2,679 sexual-minority young adults, sexual-minority participants had 4.52 times the adjusted odds of reporting suicidal ideation in the previous year. Among Catholic respondents, sexual-minority Catholics had 5.25 times the adjusted odds of suicidal ideation compared with heterosexual Catholics.",
        "Other work focused specifically on religious conflict has found relationships among unresolved conflict, internalised homophobia and suicidality. Among sexual-minority Latter-day Saints, interpersonal religious struggles, internalised homonegativity and concealment were associated with greater suicidal ideation, while resolving conflict between sexual and religious identities, family support and friend support were associated with lower suicidal ideation. The practical lesson is not that faith must be abandoned. It is that persistent shame, isolation and identity conflict should be treated as genuine safeguarding and mental-health concerns."
      ]
    },
    {
      heading: "Doctrine becomes most harmful when it is enforced through power, punishment or dependency",
      paragraphs: [
        "A theological belief and a coercive practice are not the same thing. People in a religious community may hold traditional beliefs about sex or marriage without bullying an LGBTQ+ person. Risk rises when leaders use those beliefs to justify humiliation, public exposure, threats of damnation, forced confession, pressure to enter heterosexual relationships, exclusion from family or community, intrusive monitoring, or so-called conversion practices.",
        "Institutional power matters because a church may control far more than Sunday worship. For some people it is also their friendship network, family culture, housing, university society, employer, immigration support, source of childcare or route to marriage. When belonging across several parts of life depends on obedience, apparently voluntary choices can become heavily constrained. A trauma-informed safeguarding approach therefore asks not only what doctrine was taught, but what happened when the person said no, disclosed an identity, sought outside help or tried to leave."
      ]
    },
    {
      heading: "Catholic teaching illustrates the difference between official language and lived experience",
      paragraphs: [
        "The Human Rights Campaign's overview of Roman Catholic teaching notes that the Catechism describes homosexual acts as morally impermissible and calls gay and lesbian people to chastity, while also stating that they should be treated with respect and that unjust discrimination should be avoided. The same source emphasises that lived experience varies greatly between parishes and dioceses: some communities attempt to welcome LGBTQ+ Catholics within doctrinal limits, while others have excluded members or employees because of same-sex relationships.",
        "That gap between formal statements of dignity and everyday treatment is important. A church may say that it loves LGBTQ+ people while still creating conditions in which they are expected to remain silent about partners, hide their identity, accept public teaching that depicts their relationships as uniquely immoral, or fear losing ministry, employment or community status. Safeguarding should evaluate the lived effect of policies and practices, not simply whether leaders use the language of compassion."
      ]
    },
    {
      heading: "Affirming faith can be protective, which means exclusion is not an inevitable feature of Christianity",
      paragraphs: [
        "The research also warns against treating Christianity itself as a single risk factor. A study of sexual-minority university students found that affiliation with denominations affirming same-sex marriage reduced the harmful relationship between discrimination and depressive symptoms compared with non-affirming or secular affiliation. Another population-based study of 31,852 high-school students, including 1,413 LGB students, found that living in a more supportive religious climate was associated with fewer alcohol-abuse symptoms among LGB youth (OR 0.58, 95% CI 0.40–0.85) and fewer sexual partners (OR 0.77, 95% CI 0.60–0.99).",
        "Qualitative work with Black sexual-minority men has also described LGBT-affirming churches as spaces of acceptance, emotional healing and models of loving same-sex relationships. These findings matter because they show that the choice facing an LGBTQ+ person does not have to be 'faith or mental health'. Safer churches can preserve spirituality while removing humiliation, coercion and identity-based exclusion."
      ]
    },
    {
      heading: "What churches can do differently",
      paragraphs: [
        "The most immediate safeguards are behavioural. Do not disclose someone's sexuality or gender identity without consent. Do not make an individual's private life sermon material. Do not present mental illness, suicidality or distress as proof of demonic influence or moral failure. Do not pressure a person into conversion practices, heterosexual relationships, celibacy promises or public confession as a condition of receiving pastoral care. Make complaint routes genuinely independent of the leader being complained about.",
        "Pastoral support should also recognise that some LGBTQ+ people want to remain Christian, some want to find an affirming church, and others need distance from religion entirely. A helping professional, pastor or peer supporter should not predetermine which outcome is morally correct for the person. The safer goal is restored agency: enough psychological and social freedom for the individual to decide what faith, relationships and identity will mean in their own life."
      ]
    },
    {
      heading: "Limits of the evidence",
      paragraphs: [
        "Much of the quantitative evidence comes from the United States, and several studies focus on particular denominations, age groups or sexual-minority populations rather than the full LGBTQ+ spectrum. Cross-sectional studies can identify associations but cannot prove that a religious exposure caused a later mental-health outcome. Studies also differ in how they define religiosity, affirmation, identity conflict and internalised stigma.",
        "These limitations do not erase the pattern; they tell us how carefully to state it. The most defensible conclusion is that non-affirming religious environments can function as minority-stress contexts, especially when they produce shame, concealment, exclusion or unresolved identity conflict. At the same time, affirming religious environments and constructive spiritual coping can provide belonging and resilience."
      ]
    }
  ],
  practicalOptions: [
    "If you are LGBTQ+ and still attending a church that feels unsafe, identify at least one person outside the church who knows what is happening.",
    "Keep pastoral counselling voluntary. You are allowed to end a conversation, refuse disclosure and seek an independent therapist or adviser.",
    "If leaders use private information publicly, document what happened and review the church's safeguarding, privacy and complaints procedures.",
    "Treat threats of shunning, housing loss, employment loss, forced outing or family retaliation as practical safety issues, not merely theological disagreements.",
    "If you want to keep your faith, look for communities that explicitly state how LGBTQ+ members, couples, clergy, volunteers and young people are treated in practice … not only that 'everyone is welcome'.",
    "If you are supporting someone, avoid debating theology while they are in crisis. Focus first on safety, sleep, social support, housing, health care and the person's own choices.",
    "For church leaders: prohibit conversion practices, forced confession and public outing; establish confidential safeguarding routes and train leaders in LGBTQ+-affirming mental-health literacy.",
    "If there is immediate danger, sexual violence, abuse of a child or adult at risk, or a serious suicide risk, use appropriate emergency, medical or safeguarding services rather than relying only on internal church processes."
  ],
  furtherReading: [
    {
      label: "Systematic review … Religiosity/spirituality and mental health for LGBTQIA+ people of faith",
      note:
        "A preregistered systematic review of 55 quantitative studies (N = 512,551), finding religion/spirituality can operate as both a stressor and a source of resilience depending on stigma, identity conflict, coping and affirmation.",
      url: "https://pubmed.ncbi.nlm.nih.gov/41592285/"
    },
    {
      label: "Religious conflict, sexual identity and suicidal behaviours among LGBT young adults",
      note:
        "National young-adult study examining religious identity conflict, internalised homophobia and suicidality.",
      url: "https://pmc.ncbi.nlm.nih.gov/articles/PMC4706071/"
    },
    {
      label: "Questions of faith … religious affiliation and suicidal ideation among sexual-minority young adults",
      note:
        "Large study reporting elevated adjusted odds of suicidal ideation among sexual-minority young adults across several religious affiliations.",
      url: "https://pmc.ncbi.nlm.nih.gov/articles/PMC10334798/"
    },
    {
      label: "Coming out as LGBTQ+ … mental-health impact of religious responses to disclosure",
      note:
        "Mixed-methods study of 429 participants examining suppression, shame, rejection, separation from religion and affirmation.",
      url: "https://pubmed.ncbi.nlm.nih.gov/39853193/"
    },
    {
      label: "Discrimination, depressive symptoms and gay-affirming religious affiliation",
      note:
        "Study of university students finding affirming religious affiliation could buffer the association between discrimination and depressive symptoms among sexual-minority youth.",
      url: "https://pubmed.ncbi.nlm.nih.gov/25119387/"
    },
    {
      label: "Religious climate and health-risk behaviours in sexual-minority youth",
      note:
        "Population-based study of 31,852 high-school students examining county-level supportive religious climate and health outcomes.",
      url: "https://pmc.ncbi.nlm.nih.gov/articles/PMC3489382/"
    },
    {
      label: "HRC Foundation … Stances of Faiths on LGBTQ+ Issues: Roman Catholic Church",
      note:
        "Background on official Catholic teaching and variation in LGBTQ+ parishioners' lived experiences.",
      url: "https://www.hrc.org/resources/stances-of-faiths-on-lgbt-issues-roman-catholic-church"
    }
  ]
};
