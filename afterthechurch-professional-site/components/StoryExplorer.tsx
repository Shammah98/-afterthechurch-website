...
export default function StoryExplorer({ stories, categories }: { stories: PublicStory[]; categories: readonly string[] }) {
  ...
  function reset() {
    setCategory("all"); setIntensity("all"); setMediaType("all"); setIdentity("all"); setMaxMinutes("all"); setBackground("all"); setRegion("all"); setSummaryOnly(false); setSort("recent"); setSearch(""); setVisible(12);
  }
  ...
  return (
    <div className="storyExplorer storyExplorer--youtube">
      <section className="youtubeStoryToolbar" aria-label="Filter survivor stories">
        <label className="youtubeSearch"><span className="srOnly">Search stories</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search stories" /></label>
        <select value={category} onChange={(event) => setCategory(event.target.value)} aria-label="Topic"><option value="all">All topics</option>{categories.map((item) => <option value={item} key={item}>{item}</option>)}</select>
        <select value={sort} onChange={(event) => setSort(event.target.value as SortMode)} aria-label="Sort stories"><option value="recent">Newest</option><option value="most_read">Most read</option><option value="shortest">Shortest</option></select>
        <details className="youtubeMoreFilters"><summary>More filters</summary><div className="youtubeMoreFiltersGrid">
      ...
      <div className="storyResults storyResults--youtube">
        ...
        {filtered.length === 0 ? (
          ...
          <><div className="storyList storyList--youtube">{filtered.slice(0, visible).map((story) => (
            <Link className="storyVideoCard" href={`/stories/${story.id}`} key={story.id}>
              <div className="storyVideoThumbnail"><img src={story.imageUrl || fallbackImage} alt={story.imageUrl ? `Image for ${story.title}` : "Ornate church interior used as the default image for a survivor story."} /><span className="storyDuration">{story.readingMinutes} min</span><span className={`storyIntensityBadge ${story.contentIntensity}`}>{story.contentIntensity}</span></div>
              <div className="storyVideoMeta"><h2>{story.title}</h2><p>{story.authorDisplay} · {story.churchDisplay}</p><small>{story.mediaType}{story.viewCount > 0 ? ` · ${story.viewCount} ${story.viewCount === 1 ? "read" : "reads"}` : ""}</small></div>
            </Link>
          ))}</div>{visible < filtered.length && <button type="button" className="button secondary loadMore" onClick={() => setVisible((value) => value + 12)}>Show more stories</button>}</>
        )}
      </div>
    </div>
  );
